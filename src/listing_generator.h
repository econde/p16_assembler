...
class Listing_generator: public Visitor {
	...
	void visit(Moves *s) {
		cout << s->location.line << "\t" << s->Statement::to_string()
			 << "movs\t" << register_name(s->rd->n)
				<< ", " << register_name(s->rn->n) << endl;
	}
	
	void visit(Msr *s) {
		cout << s->location.line << "\t" << s->Statement::to_string()
			 << "msr\t" << register_special_name(s->rs->n)
				<< ", " << register_name(s->rd->n) << endl;
	}

	void visit(Mrs *s) {
		cout << s->location.line << "\t" << s->Statement::to_string()
			 << "mrs\t" << register_name(s->rd->n)
				<< ", " << register_special_name(s->rs->n) << endl;
	}
